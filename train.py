...
def validate_epoch(model, val_loader, criterion, device):
    ...
    with torch.no_grad():
        ...
        for data, target in val_pbar:
            ...
            # Use autocast for mixed precision validation (optional, but can provide some speedup)
            with autocast():
                output = model(data)
...
if __name__ == '__main__':
    ...
    freeze_support()  # This is required for Windows
    ...
    with mlflow.start_run(run_name=run_id) as run:
        ...
        # Create a progress bar for epochs in Phase 1
        epoch_pbar = tqdm(range(config['epochs_head']), desc="Phase 1 Epochs")
        ...
        # Create a progress bar for epochs in Phase 2
        epoch_pbar2 = tqdm(range(config['epochs_fine_tune']), desc="Phase 2 Epochs")
        for epoch in epoch_pbar2:
